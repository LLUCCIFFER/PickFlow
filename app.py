...
def _require_engine(engine: str) -> None:
    """Only the local fast engine is available in this edition."""
    if engine != "fast":
        raise ValueError("当前版本只支持极速模式")
...
# ============================================================
# 韩式大头贴 / 网感滤镜导出：给 winners 做风格化副本
# ============================================================

@dataclass
class BeautifyJobState:
    status: str = "idle"
    done: int = 0
    total: int = 0
    current: str = ""
...
BEAUTIFY_JOB: Optional[BeautifyJobState] = None


@app.route("/api/beautify/styles")
def api_beautify_styles():
    from pic_selecter.photobooth import list_styles
    return jsonify({"styles": list_styles()})


@app.route("/api/beautify/preview", methods=["POST"])
def api_beautify_preview():
    if SESSION is None:
        return jsonify({"error": "no session"}), 400
    winners = _winner_paths()
    if not winners:
    ...
    from pic_selecter.photobooth import PhotoboothConfig, list_styles, render
    cfg_dict = request.get_json(silent=True) or {}
    cfg = PhotoboothConfig.from_dict(cfg_dict)
    try:
        idx = int(cfg_dict.get("preview_index", 0))
    except (ValueError, TypeError):
        idx = 0
    ...
    try:
        ...
        logger.exception("beautify preview failed")
        return jsonify({"error": f"{type(e).__name__}: {e}"}), 500

    import base64
    styles = {s["id"]: s for s in list_styles()}
    style_info = styles.get(cfg.style, styles["seoul_booth"])
    return jsonify({
        "image_b64": base64.b64encode(data).decode("ascii"),
        "size_kb": round(len(data) / 1024, 1),
        "source_name": Path(src).name,
...
def _run_beautify_job(src_paths: list[str], dst: Path, cfg) -> None:
    global BEAUTIFY_JOB
    job = BEAUTIFY_JOB
    assert job is not None
    from pic_selecter.photobooth import batch_export
    ...
    try:
        ...
        logger.info(
            f"beautify: 完成 ok={result['ok']} failed={len(result['failed'])} out={dst}"
        )
    except Exception as e:
        logger.exception("beautify batch error")
        job.status = "error"
        job.error = f"{type(e).__name__}: {e}"
        job.finished_at = time.time()


@app.route("/api/beautify/start", methods=["POST"])
def api_beautify_start():
    global BEAUTIFY_JOB
    if SESSION is None:
        return jsonify({"error": "no session"}), 400
    if BEAUTIFY_JOB and BEAUTIFY_JOB.status == "running":
        return jsonify({"error": "已有美化导出任务在跑"}), 409

    winners = _winner_paths()
    if not winners:
        return jsonify({"error": "没有 winner 照片可导出"}), 400

    from pic_selecter.photobooth import PhotoboothConfig
    cfg = PhotoboothConfig.from_dict(request.get_json(silent=True) or {})

    stamp = time.strftime("%Y%m%d_%H%M%S")
    out_dir = winners_dir(SESSION.folder) / f"kstyle_{stamp}"
    out_dir.mkdir(parents=True, exist_ok=True)

    BEAUTIFY_JOB = BeautifyJobState(
        status="running",
        total=len(winners),
        out_dir=str(out_dir),
        started_at=time.time(),
...
@app.route("/api/beautify/status")
def api_beautify_status():
    if BEAUTIFY_JOB is None:
        return jsonify({"status": "idle"})
    j = BEAUTIFY_JOB
    return jsonify({
        "status": j.status,
...
@app.route("/api/beautify/cancel", methods=["POST"])
def api_beautify_cancel():
    if BEAUTIFY_JOB is None or BEAUTIFY_JOB.status != "running":
        return jsonify({"ok": False, "error": "no running job"}), 400
    BEAUTIFY_JOB.cancel_requested = True
    return jsonify({"ok": True})


@app.route("/api/beautify/open_out_dir", methods=["POST"])
def api_beautify_open_out_dir():
    if BEAUTIFY_JOB is None or not BEAUTIFY_JOB.out_dir:
        return jsonify({"error": "no output dir"}), 400
    target = Path(BEAUTIFY_JOB.out_dir)
    if not target.exists():
        return jsonify({"error": "目录不存在"}), 400
    try:
        if sys.platform == "darwin":
